...
// .wrangler/tmp/bundle-M8jogi/checked-fetch.js
...
// .wrangler/tmp/pages-HSRq8C/functionsWorker-0.5389386059477079.mjs
...
var onRequest15 = /* @__PURE__ */ __name2(async (context) => {
  const { env } = context;
  try {
    const tokenStmt = await env.DB.prepare("SELECT * FROM tokens LIMIT 1").first();
    if (!tokenStmt) {
      return new Response("No token found in DB", { status: 500 });
    }
    const accessToken = tokenStmt.access_token;
    const shopId = tokenStmt.shop_id;
    const partnerId = env.SHOPEE_PARTNER_ID;
    const partnerKey = env.SHOPEE_PARTNER_KEY;
    if (!accessToken || !shopId) {
      return new Response("Invalid token data in DB", { status: 500 });
    }
    const logs = [];
    const results = {};
    ...
    const findCategory = /* @__PURE__ */ __name2((cats, keywords) => {
    ...
    for (const target of TARGET_BRANDS) {
      const match2 = brandList.find(
        (b) => b.display_brand_name.toLowerCase() === target.toLowerCase() || b.original_brand_name.toLowerCase() === target.toLowerCase()
      );
      if (match2) {
        foundBrands.push({ target, ...match2 });
      } else {
...
async function getCategories(partnerId, partnerKey, accessToken, shopId) {
  const path = "/api/v2/product/get_category";
  const timestamp = Math.floor(Date.now() / 1e3);
  const baseString = `${partnerId}${path}${timestamp}${accessToken}${shopId}`;
  const sign = await hmacSha25612(partnerKey, baseString);
  const url = `${SHOPEE_HOST12}${path}?partner_id=${partnerId}&timestamp=${timestamp}&access_token=${accessToken}&shop_id=${shopId}&sign=${sign}&language=en`;
...
async function getBrandList(partnerId, partnerKey, accessToken, shopId, categoryId) {
  ...
  while (hasNextPage) {
    const url = `${SHOPEE_HOST12}${path}?partner_id=${partnerId}&timestamp=${timestamp}&access_token=${accessToken}&shop_id=${shopId}&sign=${sign}&category_id=${categoryId}&status=1&offset=${offset}&page_size=${pageSize}&language=en`;
    const res = await fetch(url);
    const data = await res.json();
    ...
    if (allBrands.length > 2e3) break;
  }
  return allBrands;
}
__name(getBrandList, "getBrandList");
__name2(getBrandList, "getBrandList");
...
// .wrangler/tmp/bundle-M8jogi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-M8jogi/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5389386059477079.js.map
